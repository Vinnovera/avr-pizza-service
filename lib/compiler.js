...
function Compiler(request) {
  ...
  const downloadBasePath = path.join(__dirname, '..', 'bin', 'download', this.builderDir);
  ...
  this.paths = {
    ...
    downloadedLibs: path.join(downloadBasePath, 'libraries')
  };
}
...
Compiler.prototype.buildSketch = function(callback) {
  ...
  const builderString = [
    ...
    `-libraries="${self.paths.downloadedLibs}"`,
